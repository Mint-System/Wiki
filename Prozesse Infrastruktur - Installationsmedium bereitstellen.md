...
```
- From the unpacked iso, move the '[BOOT]' folder to its parent folder. eg from $HOME/autoinstaller/install-iso/[BOOT] to $HOME/autoinstaller/[BOOT]
and rename [BOOT] to BOOT, for example in one command using:
~~~
...
```
- From the output copy all after "-V 'ubuntu ......" and now we have to adjust a few lines and add '/' at the end of each line (except the last). These are marked by '\*' in this example, after the # is written what to change:
```
